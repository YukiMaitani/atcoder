...
def main():
  ...
  data = imosCreate(clouds, c, c)
  data = tdCumsum(data)
  single = tdBinaryFilter(data, lambda x: x == 1)
  single = tdCumsum(single)
  empty = tdCount(data, lambda x: x == 0, h=c, w=c)
  for cloud in clouds:
    print(empty + tdCumsumArea(single, cloud))
